...
log(`\n${c.b}── O que mudou na v1.8.1 ──────────────────────────────────────${c.x}`)
log(`${c.g}px-handoff${c.x} — as travas antigas contradiziam o caminho do FONTE:`)
log(`${c.d}  · A v1.8.0 passou a entregar o fonte do proto quando a stack do dev é a mesma,${c.x}`)
log(`${c.d}    mas description, DoD e manifest ainda diziam "nunca envia código-fonte".${c.x}`)
log(`${c.d}  · Agora os três estão alinhados: o que nunca sai é config de build${c.x}`)
log(`${c.d}    (vite.config, tsconfig, package.json, .env) e src/components/ui/**,${c.x}`)
log(`${c.d}    que vem do registry @centralit em vez de duplicar biblioteca no pacote.${c.x}`)
log(`${c.d}  · DoD passou a ter um item por caminho: FONTE (stack igual) e${c.x}`)
log(`${c.d}    REFERÊNCIA VISUAL (stack diferente). Antes só existia o segundo.${c.x}`)
log(`\n${c.y}Se você está vindo da v1.6.x, a mudança grande é a v1.8.0:${c.x}`)
log(`${c.d}  o handoff entrega o FONTE do protótipo (componentes reais + tokens) quando o dev${c.x}`)
log(`${c.d}  implementa na mesma stack. Fidelidade estrutural em vez de screenshot. Ver CHANGELOG.${c.x}`)
