...
- **O repo do dev já existe** → **clonar o repo do dev** e criar a branch `ux/<projeto>` (Passo 3). O UX idealiza direto na arquitetura real.
...
2. Criar/entrar na branch de UX: `git checkout -b ux/<projeto>` (recém-clonado já parte da `main` atualizada; se o repo já estava na máquina, `git checkout main && git pull` antes).
3. **Nunca trabalhar na `main`.** Todo trabalho de UX vive numa branch `ux/<projeto>`.
4. Encaminhar pra idealização (`/px-start`) se ainda não passou por ela.

## Passo 4 — Entrega: levar o sandbox pro repo do dev

Roda quando o dev libera o repo e o UX já tem trabalho no sandbox.
...
**Executar:**
1. Garantir o repo do dev clonado e a branch `ux/<projeto>` criada (Passo 3).
...
3. Commitar e empurrar a branch: `git add .` → `git commit -m "ux(<projeto>): ..."` → `git push -u origin ux/<projeto>`.
4. Gerar o **link do Merge Request** no GitLab (`ux/<projeto>` → `main`) pra o UX abrir; o dev revisa e integra.
...
- **Nunca commit na `main`.** Sempre branch `ux/<projeto>` + Merge Request.
...
px-setup (terreno / entrega)
   ├─ monta sandbox  ──►  /px-start  →  px-intake  →  px-kickoff  →  px-request  →  px-story
   └─ entrega (Passo 4)  ◄── quando o dev libera o repo → branch ux/ + Merge Request → dev
```
